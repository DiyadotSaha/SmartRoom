...
room_data = '/Users/asad/SmartRoom/Room2.csv'
...
def brute_force(df, duration_minutes, output_csv):
    print(f"Running brute simulation for {duration_minutes} minutes...")
...
def simulate_passive_temperature(df, duration_minutes, output_csv):
    print(f"Running passive simulation for {duration_minutes} minutes...")
    start_time = df['time'].min()
    end_time = min(df['time'].max(), start_time + duration_minutes * 60 * 1000)
    current_time = start_time
    room_temp = df.iloc[0]['temperature']
...
def linear_reg(df, duration_minutes, output_csv):
    print(f"Running smart simulation linear regression for {duration_minutes} minutes...")
    start_time = df['time'].min()
    end_time = min(df['time'].max(), start_time + duration_minutes * 60 * 1000)
    current_time = start_time
    room_temp = df.iloc[0]['temperature']
    ...
    while current_time <= end_time:
        ...
        if not window.empty:
            ...
            publish_data_command(command=json.dumps(data).encode('utf-8'), topic='room_2_UI')
            publish_HVAC_command(command=command.encode('utf-8'), topic='room_2_HVAC')
